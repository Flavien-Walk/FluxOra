...
import { Plus, AlertTriangle, BarChart2, Send, Home, Users, List, BookOpen, Bell, ArrowRight, Loader2 } from 'lucide-react';
      ...
      {actions.map((action) => {
        ...
        return (
          <button
            ...
            disabled={isDisabled}
            className={`flex items-center justify-between w-full rounded-xl px-3.5 py-2.5 text-[12.5px] font-medium border transition-colors text-left group ${style} disabled:opacity-60 disabled:cursor-not-allowed`}
          >
            <div className="flex items-center gap-2">
              ...
              <span>{isRunning ? 'En cours…' : action.label}</span>
            </div>
            {!isRunning && (
