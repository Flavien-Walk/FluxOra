...
export function SkeletonTable({ rows = 5, cols = 5 }) {
  return (
    <div className="overflow-hidden">
      <div className="bg-slate-50/60 border-b border-slate-100 px-4 py-3 flex gap-8">
        {Array.from({ length: cols }).map((_, i) => (
  ...
  return (
    <div className="flex-1 p-6 space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 bg-white rounded-xl border border-slate-200 p-5 h-52" />
        <div className="bg-white rounded-xl border border-slate-200 p-5 h-52" />
      </div>
    </div>
  );
}
